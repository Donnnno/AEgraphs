...
function createColumnChart() {
    ...
    try {
        ...
        sliderDistance.property("ADBE Slider Control-0001").expression = "var targetName = \"Balk\";\nvar balkAantal = 0;\nfor (var i = 1; i <= thisComp.numLayers; i++) {\n    if (thisComp.layer(i).name.indexOf(targetName) !== -1) {\n        balkAantal++;\n    }\n}\nvar availableWidth = thisComp.layer(\"Controller\").effect(\"Lijnen - Lengte\")(\"Slider\") - 40; // 20px margin on each side\nvar minGap = 20; // Minimum gap between bars\nvar barWidth = thisComp.layer(\"Controller\").effect(\"Balk - Breedte\")(\"Slider\");\nvar totalBarWidth = barWidth * balkAantal;\nvar totalGapWidth = minGap * (balkAantal - 1);\nvar remainingWidth = availableWidth - totalBarWidth - totalGapWidth;\nvar spacing = barWidth + minGap + (remainingWidth / (balkAantal - 1));\nspacing;";

        var sliderWidth = shapeController.property("ADBE Effect Parade").addProperty("ADBE Slider Control");
        sliderWidth.name = "Balk - Breedte";
        sliderWidth.property("ADBE Slider Control-0001").expression = "var targetName = \"Balk\";\nvar balkAantal = 0;\nfor (var i = 1; i <= thisComp.numLayers; i++) {\n    if (thisComp.layer(i).name.indexOf(targetName) !== -1) {\n        balkAantal++;\n    }\n}\nvar availableWidth = thisComp.layer(\"Controller\").effect(\"Lijnen - Lengte\")(\"Slider\") - 40; // 20px margin on each side\nvar minGap = 20; // Minimum gap between bars\nvar totalGapWidth = minGap * (balkAantal - 1);\nvar availableForBars = availableWidth - totalGapWidth;\nvar optimalWidth = availableForBars / balkAantal;\nvar width = Math.max(50, Math.min(200, optimalWidth));\nwidth;";
